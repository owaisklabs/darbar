            ...
            <div class="form-group mt-4">
                ...
                <select name="category" required="" class="form-control select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="1" tabindex="-1" aria-hidden="true">
                    ...
                    @foreach($categories as $category)
                        <option value="{{$category->id}}" @if($category->id == $product->category_id) selected @else " " @endif>{{$category->name}} </option>
                    @endforeach
                </select>
            </div>
            <div class="form-group mt-4">
                <label for="exampleInputEmail1">Product Name</label>
                <input type="text" value="{{$product->name}}" required="" name="product_name" class="form-control" id="exampleInputEmail1" placeholder="Past Paper Title">
            </div>
